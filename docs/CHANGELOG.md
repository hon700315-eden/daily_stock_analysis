...
- [修復] 收斂 API SQLite 分析歷史 DB 路徑契約，相對路徑固定解析到 repository root，並新增 readiness 與 history unavailable 防禦，避免空 DB、缺表或 malformed DB 被回成成功空列表。
...
- [修復] 收斂 Portfolio 缺行情公開契約，缺價持倉的現價、市值與未實現損益改以 `null` 回傳並保留既有快取相容。
- [文檔] 新增台股 Portfolio 帳務契約稽核交接文件，記錄實際帳本、成本算法、行情來源與未實作邊界。
- [修復] 修正台股新聞搜尋與 LLM 報告語境，台股查詢改用台灣繁中來源提示並避免混入中國市場公告與人民幣語境。
- [修復] 修正台股籌碼分布不再嘗試中國市場資料源，並新增法人與籌碼資料稽核交接文件。
- [修復] 收斂台股財報與公司事件稽核邊界，避免缺少結構化財務資料時在 Prompt 渲染空財報表格。
- [修復] 修正 Docker 與每日分析 workflow 的台股正式預設時區、市場區域與紅漲綠跌配色。
- [文檔] 新增台股最終清理與部署驗收交接文件，記錄中國語境殘留裁決、正式資料 readback、API、Web、Docker、workflow 與不可用項目。
